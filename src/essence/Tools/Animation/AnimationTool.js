                    ...
                    "<div class='panel-content'>",
                        ...
                        "<div class='time-controls'>",
                            ...
                            "<div class='step-selector'>",
                                "<label>Step:</label>",
                                "<input type='number' id='timeStep' min='0' max='99' step='0.5' value='1'>",
                            "</div>",
                    ...
                    "<div class='panel-content'>",
                        ...
                        "<div class='control-group'>",
                            ...
                            "<div class='slider-container'>",
                                "<input type='range' id='frameRateSlider' min='0.05' max='10' step='0.05' value='1'>",
                                "<span class='slider-value' id='frameRateValue'>Every 1s</span>",
                            "</div>",
                        "</div>",
                        "<div class='control-group'>",
                        ...
                        "<div class='control-group'>",
                            ...
                            "<input type='text' id='exportTitle' placeholder='Enter animation title'>",
                        "</div>",
                        "<div class='control-group'>",
                            "<div class='checkbox-group'>",
                        ...
                        "<div class='control-group'>",
                            ...
                            "<div class='slider-container'>",
                                "<input type='range' id='layerRefreshRateSlider' min='100' max='20000' step='100' value='1000'>",
                                "<span class='slider-value' id='layerRefreshRateValue'>1s</span>",
                            "</div>",
                        "</div>",
                    "</div>",
                "</div>",
                    ...
                    "<div class='panel-content'>",
                        ...
                        "<div id='animationProgress' style='display: none; margin-top: 20px;'>",
                            ...
                            "<div style='background: #2a2a2a; border-radius: 4px; padding: 3px; margin-bottom: 10px;'>",
                                ...
                                "</div>",
                            "</div>",
                            "<div id='animationProgressStatus' style='font-size: 13px; color: #888; text-align: center;'></div>",
                        "</div>",
                        "<div id='animationDebugPreview' style='display: none; margin-top: 20px; border: 2px solid #4CAF50; border-radius: 4px; padding: 10px; background: #1a1a1a;'>",
                            "<h4 style='margin: 0 0 10px 0; color: #4CAF50; font-size: 14px;'>Preview: Latest Captured Frame</h4>",
                            "<canvas id='animationDebugCanvas' style='display: block; width: 100%; height: auto; border: 1px solid #333;'></canvas>",
                            "<div id='animationDebugInfo' style='margin-top: 8px; font-size: 11px; color: #888; text-align: center;'></div>",
                        "</div>",
                    "</div>",
                "</div>",
            "</div>",
...
const AnimationTool = {
    ...
    animationSettings: {
        ...
        showTimeStep: false,
        showScaleBar: false,
        layerRefreshRate: 1000, // Delay after updating time to wait for layers to update (in milliseconds)
    },
    cachedImages: [],
    isPlaying: false,
    currentFrame: 0,
...
function interfaceWithMMGIS() {
    ...
    function initializeComponents() {
        ...
        // Initialize frame rate slider to default value (overrides sync with AnimationTool defaults)
        const defaultFrameRate = 1 // Default: 1 FPS = 1 second per frame
        AnimationTool.animationSettings.frameRate = defaultFrameRate
        $('#frameRateSlider').val(defaultFrameRate)
        $('#frameRateValue').text(fpsToEverySeconds(defaultFrameRate))

        // Initialize layer refresh rate slider
        const defaultLayerRefreshRate = 1000 // Default: 1 second
        AnimationTool.animationSettings.layerRefreshRate =
            defaultLayerRefreshRate
        $('#layerRefreshRateSlider').val(defaultLayerRefreshRate)
        const initialSeconds = (defaultLayerRefreshRate / 1000).toFixed(1)
    ...
    function syncWithTimeUIAnimation() {
        ...
        if (
            Math.abs(newFPS - AnimationTool.animationSettings.frameRate) > 0.01
        ) {
    ...
    function syncTimeUIFromAnimation() {
        ...
        // Update TimeUI Step
        const newStepIndex =
            timeIntervalToStep[AnimationTool.animationSettings.timeInterval] ||
            3
        ...
        const newIntervalIndex =
            fpsToIntervalMapping[AnimationTool.animationSettings.frameRate] || 2
    ...
    function setupEventHandlers() {
        ...
        $('#timeInterval').on('change', () => {
            AnimationTool.animationSettings.timeInterval =
                $('#timeInterval').val()
        ...
        // NOTE: Do NOT update visual rectangle on pan/zoom
        // The polygon is already in geographic coordinates, so Leaflet
        // will automatically render it in the correct location.
        // Only the screenRect (for offscreen positioning) needs updating on zoom.
        Map_.map.on('zoomend', () => {
            if (AnimationTool.boundingBox && AnimationTool.screenRect) {
                // Recalculate screenRect for offscreen map positioning
                // but don't redraw the visual polygon
                ...
                // Do NOT call updateVisualRectangle() - let Leaflet handle polygon rendering
            }
        })
    }
    ...
    function updateStepDisplay() {
        ...
        $(`.animation-step[data-step="${AnimationTool.currentStep}"]`).addClass(
            'active'
        )
    ...
    function enableDrawingMode() {
        ...
        showDrawingInstructions(
            'Click once to set the first corner, then click again to set the opposite corner.'
        )
    ...
    function setupCustomRectangleDrawing() {
        ...
        const onClick = (e) => {
            ...
            if (!hasStarted) {
                ...
                if (AnimationTool.tempRectangle) {
                    AnimationTool.drawingLayer.removeLayer(
                        AnimationTool.tempRectangle
                    )
                }
                ...
                if (
                    !AnimationTool.drawingStartPoint ||
                    !AnimationTool.tempRectangle
                )
                    return
                ...
                const bottomLeft = map.containerPointToLatLng({
                    x: minX,
                    y: maxY,
                })
        ...
        const onMouseMove = (e) => {
            ...
            const minX = Math.min(
                AnimationTool.drawingStartPoint.x,
            ...
            const minY = Math.min(
                AnimationTool.drawingStartPoint.y,
    ...
    function cleanupCustomRectangleDrawing() {
        ...
        if (
            ...
            AnimationTool.originalCursor !== undefined
        ) {
        ...
        if (AnimationTool.drawingHandlers) {
            ...
            document.removeEventListener(
                'keydown',
                AnimationTool.drawingHandlers.keydown
            )
    ...
    function showDrawingInstructions(
        message = 'Click and drag to draw a rectangle. Press ESC to cancel.'
    ) {
    ...
    function updateVisualRectangle() {
        ...
        const bottomRight = Map_.map.containerPointToLatLng({
            x: maxX,
            y: maxY,
        })
    ...
    function updateTimeControlFromAnimation() {
        if (!TimeControl || !TimeControl.enabled || !AnimationTool.timeRange)
            return
        ...
        generateAnimationFrames()
            .then((frames) => {
                if (frames.length === 0) {
                    ...
                    showModalAlert(
                        'No frames generated. Please check your time range and settings.'
                    )
                    return
                }
    ...
    function generateAnimationFrames() {
        return new Promise(async (resolve, reject) => {
            ...
            try {
                ...
                // Initialize offscreen map for background rendering
                try {
                    if (!AnimationTool.offscreenManager) {
                        AnimationTool.offscreenManager =
                            new OffscreenMapManager(
                                Map_.map,
                    ...
                    await AnimationTool.offscreenManager.initialize(
                        ...
                        screenRect.width,
                        screenRect.height,
                        screenRect // Pass full screenRect for accurate positioning in projections
                    )
                    ...
                    console.error(
                        'Failed to initialize offscreen map, using main map fallback:',
                        offscreenError
                    )
                ...
                // Show progress UI
                showProgressUI(timeSteps.length)
                ...
                const generateFrame = () => {
                    ...
                    // Keep the drawing layer (red bounding box) visible during frame capture
                    // Users want to see where they drew the bbox while the animation generates
                    // The offscreen map doesn't include this layer, so it won't appear in the final animation
                    if (
                        currentStep === 0 &&
                        AnimationTool.drawingLayer &&
                        Map_.map.hasLayer(AnimationTool.drawingLayer)
                    ) {
                        drawingLayerWasVisible = true
                        // REMOVED: Map_.map.removeLayer(AnimationTool.drawingLayer)
                        // The bbox stays visible on main map but doesn't appear in offscreen captures
                    }
                    ...
                    captureMapFrame(timeStep, interval)
                        ...
                        .catch((error) => {
                            ...
                            if (
                                ...
                                AnimationTool.drawingLayer.addTo(Map_.map)
                                console.log(
                                    'Restored drawing layer after error'
                                )
                            }
                            reject(error)
                        })
    ...
    function showProgressUI(totalFrames) {
        const progressDiv = $('#animationProgress')
        const previewDiv = $('#animationDebugPreview')
    ...
    function updateProgress(current, total, status) {
        ...
        $('#animationProgressBar').css('width', percent + '%')
        $('#animationProgressText').text(`${current} / ${total}`)
        $('#animationProgressStatus').text(
            status || `Frame ${current} of ${total}`
        )
    }
    ...
    function hideUIElementsForCapture() {
        ...
        if (showScaleBar && scaleBar) {
            ...
            hiddenElements['#scaleBar_marginTop'] =
                scaleBar.style.marginTop || ''
            hiddenElements['#scaleBar_display'] = scaleBar.style.display || ''
            hiddenElements['#scaleBar_visibility'] =
                scaleBar.style.visibility || ''

            if (scaleBarBounds) {
                hiddenElements['#scaleBarBounds_display'] =
                    scaleBarBounds.style.display || ''
                hiddenElements['#scaleBarBounds_visibility'] =
                    scaleBarBounds.style.visibility || ''
            ...
            if (mapToolBar) {
                hiddenElements['#mapToolBar_display'] =
                    mapToolBar.style.display || ''
                hiddenElements['#mapToolBar_visibility'] =
                    mapToolBar.style.visibility || ''
                ...
                toolBarChildren.forEach((child) => {
                    if (child.nodeType === 1) {
                        ...
                        if (child.id && child.id !== 'scaleBarBounds') {
                            hiddenElements[`#mapToolBar_${child.id}_display`] =
                                originalDisplay
                            hiddenElements[
                                `#mapToolBar_${child.id}_visibility`
                            ] = originalVisibility
    ...
    function restoreUIElements(hiddenElements) {
        ...
        if (scaleBar) {
            if (hiddenElements['#scaleBar_display'] !== undefined) {
                scaleBar.style.display =
                    hiddenElements['#scaleBar_display'] || ''
            }
            ...
            if (hiddenElements['#scaleBar_marginTop'] !== undefined) {
                scaleBar.style.marginTop =
                    hiddenElements['#scaleBar_marginTop'] || '5px'
            } else {
        ...
        if (mapToolBar) {
            ...
            Object.keys(hiddenElements).forEach((selector) => {
                if (
                    selector.startsWith('#mapToolBar_') &&
                    selector.endsWith('_display')
                ) {
                    const childId = selector
                        .replace('#mapToolBar_', '')
                        .replace('_display', '')
    ...
    function drawTextWithBorder(
        ...
        if (textWidth > effectiveMaxWidth) {
            adjustedFontSize = Math.floor(
                fontSize * (effectiveMaxWidth / textWidth)
            )
    ...
    function captureMapFrame(timestamp, interval) {
        return new Promise(async (resolve, reject) => {
            try {
                ...
                if (
                    !AnimationTool.offscreenManager ||
                    !AnimationTool.offscreenManager.initialized
                ) {
                    reject(new Error('Offscreen map manager not initialized'))
                    return
                }

                // Capture frame from offscreen map
                const canvas =
                ...
                if (AnimationTool.animationSettings.showScaleBar) {
                    processedCanvas =
                        AnimationTool.offscreenManager.renderScaleBarOnCanvas(
                            canvas
                        )
                }
            ...
            frames.forEach((frameData, index) => {
                ...
                img.onload = () => {
                    ...
                    if (loadedCount === frames.length) {
                        ...
                        const frameInterval =
                            1 / AnimationTool.animationSettings.frameRate // Convert FPS to seconds per frame

                        // Use actual dimensions from the first frame's image instead of screenRect
                        // This preserves the resolution of the captured frames
                            ...
                            function (obj) {
                                if (!obj.error) {
                                    ...
                                    button
                                        ...
                                        .prop('disabled', false)
                                    showModalAlert(
                                        'Animated GIF exported successfully!'
                                    )
                                } else {
                                    ...
                                    button
                                        ...
                                        .prop('disabled', false)
                                    showModalAlert(
                                        'GIF creation failed. First frame exported as PNG.'
                                    )
                                }
                            }
                        )
    ...
    function exportAsMP4(frames, button, originalText) {
        try {
            ...
            const invalidFrames = frames.filter(
                (frame) => !frame.data || !frame.data.startsWith('data:image')
            )
            ...
            try {
                ...
                throw new Error(
                    'GIF library not working properly: ' + testError.message
                )
            }
            ...
            gif.on('progress', (progress) => {
                ...
                if (progress > lastProgress) {
                    ...
                    progressTimeout = setTimeout(() => {
                        console.warn(
                            'GIF creation appears stuck, falling back...'
                        )
            ...
            frames.forEach((frameData, index) => {
                ...
                img.onload = () => {
                    try {
                        const delay =
                            1000 / AnimationTool.animationSettings.frameRate
                        ...
                        if (framesAdded === frames.length) {
                            try {
                                ...
                                progressTimeout = setTimeout(() => {
                                    if (!renderStarted) {
                                        console.warn(
                                            'GIF render appears stuck, falling back...'
                                        )
                                        createSimpleGIF(
                                            frames,
                                            button,
                                        ...
                                        progressTimeout = setTimeout(() => {
                                            console.warn(
                                                'GIF render timeout exceeded, falling back...'
                                            )
                                            createSimpleGIF(
                                                frames,
                                                button,
                ...
                img.onload = () => {
                    ...
                    if (currentFrame < frames.length) {
                        ...
                        canvas.toBlob((blob) => {
                            ...
                            downloadFile(
                                url,
                                'animation_final_frame.png',
                                'image/png'
                            )
                            button.text(originalText).prop('disabled', false)
                            showModalAlert(
                                'Animation frames exported as PNG (GIF creation unavailable)'
                            )
                        }, 'image/png')
                    }
                }
    ...
    async function exportMP4WithFFmpeg(frames, button, originalText) {
        try {
            ...
            for (let i = 0; i < frames.length; i++) {
                ...
                // Convert data URL to Uint8Array using fetchFile from @ffmpeg/util
                const uint8Array = await fetchFile(frameData)
            ...
            // Use scale filter to ensure dimensions are even (required by H.264)
            const ffmpegArgs = [
                '-framerate',
                AnimationTool.animationSettings.frameRate.toString(),
                '-i',
                'frame%03d.png',
                '-vf',
            ...
            // Clean up blob URL after download
            setTimeout(() => URL.revokeObjectURL(videoUrl), 100000)
    ...
    function resetAnimation() {
        ...
        $('#layerRefreshRateSlider').val(1000)
        $('#layerRefreshRateValue').text('1s')
        $('#resetValues').prop('disabled', true)
    ...
    function separateFromMMGIS() {
        ...
        $(
            '#frameRateSlider, input[name="playDirection"], #loopAnimation, #showTimeStep, #showScaleBar'
        ).off()
        $('#exportTitle').off()
