/**
 * StratColumnBars attachment — unit tests.
 *
...
import StratColumnBars from '../stratColumnBars.js'
import {
    manifestOf,
    unresolvedModules,
...
test('plugin.json declares a valid layerattachment contract @unit', () => {
    ...
    expect(manifest.attachmentId).toBe('strat_column_bars')
    // Settings live on the host, so the form must write where the manifest says
    // this attachment is configured.
    expect(manifest.configPath).toBe('variables.layerAttachments.stratColumnBars')
...
test('the declared module resolves and exports make @unit', () => {
    ...
    expect(typeof StratColumnBars.make).toBe('function')
})
