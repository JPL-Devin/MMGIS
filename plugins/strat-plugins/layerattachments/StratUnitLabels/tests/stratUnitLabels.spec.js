/**
 * StratUnitLabels attachment — unit tests.
 *
...
import StratUnitLabels from '../stratUnitLabels.js'
import {
    manifestOf,
    unresolvedModules,
...
test('plugin.json declares a valid layerattachment contract @unit', () => {
    ...
    expect(manifest.attachmentId).toBe('strat_unit_labels')
    // Settings live on the host, so the form must write where the manifest says
    // this attachment is configured.
    expect(manifest.configPath).toBe('variables.layerAttachments.stratUnitLabels')
...
test('the declared module resolves and exports make @unit', () => {
    ...
    expect(typeof StratUnitLabels.make).toBe('function')
})
