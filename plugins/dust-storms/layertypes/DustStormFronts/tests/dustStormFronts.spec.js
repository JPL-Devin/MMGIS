/**
 * DustStormFronts layer type — unit tests.
 *
...
import DustStormFronts from '../dustStormFronts.js'
...
test('plugin.json declares a valid extending layertype contract @unit', () => {
    ...
    expect(manifest.typeId).toBe('duststormfronts')
...
test('the module declares surfaces, not renderer operations @unit', () => {
    ...
    for (const key of Object.keys(DustStormFronts)) expect(SURFACES).toContain(key)
})
...
test('fetch returns the features the service gave it @unit', async () => {
    ...
    try {
        const result = await DustStormFronts.source.fetch(
            { name: 'DustStormFronts' },
