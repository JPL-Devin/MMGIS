/**
 * StrayField attachment — unit tests.
 *
...
import StrayField from '../strayField.js'
import {
    manifestOf,
    unresolvedModules,
...
test('plugin.json declares a valid layerattachment contract @unit', () => {
    ...
    expect(manifest.attachmentId).toBe('stray_field')
    // Settings live on the host, so the form must write where the manifest says
    // this attachment is configured.
    expect(manifest.configPath).toBe('variables.layerAttachments.strayField')
...
test('the declared module resolves and exports make @unit', () => {
    ...
    expect(typeof StrayField.make).toBe('function')
})
