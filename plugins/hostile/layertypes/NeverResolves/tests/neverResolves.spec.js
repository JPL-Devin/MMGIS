/**
 * NeverResolves layer type — unit tests.
 *
...
import NeverResolves from '../neverResolves.js'
...
test('plugin.json declares a valid extending layertype contract @unit', () => {
    ...
    expect(manifest.typeId).toBe('neverresolves')
...
test('the module declares surfaces, not renderer operations @unit', () => {
    ...
    for (const key of Object.keys(NeverResolves)) expect(SURFACES).toContain(key)
})
...
test('fetch returns the features the service gave it @unit', async () => {
    ...
    try {
        const result = await NeverResolves.source.fetch(
            { name: 'NeverResolves' },
