/**
 * RoverNote interaction — unit tests.
 *
 * Run with `npm run test:plugins:unit` (the `@unit` tag selects these;
 * `npm run test:unit` only covers `tests/unit`).
 *
 * These import `logic.js`, not `RoverNote.js`: as soon as the handler imports a
...
test('plugin.json declares a valid interaction contract @unit', () => {
    ...
    expect(manifest.interactionId).toBe('rover:note')
...
test('settings are defaulted where they are read @unit', () => {
    const feature = { properties: { rover: 'Sol-1', callsign: 'RC7' } }
    expect(decide(feature, null).rover).toBe('Sol-1')
    expect(decide(feature, { idProp: 'callsign' }).rover).toBe('RC7')
    expect(decide(feature, { notePrefix: 'Seen' }).note).toMatch(/^Seen /)
})

test('a feature without the id property decides nothing @unit', () => {
