/**
 * MissionEvents layer type — unit tests.
 *
...
test('plugin.json declares a valid extending layertype contract @unit', () => {
    ...
    expect(manifest.typeId).toBe('missionevents')
...
test('the module declares surfaces, not renderer operations @unit', () => {
    ...
    for (const key of Object.keys(MissionEvents)) expect(SURFACES).toContain(key)
})
...
test('fetch returns the features the service gave it @unit', async () => {
    ...
    try {
        const result = await MissionEvents.source.fetch(
            { name: 'MissionEvents' },
