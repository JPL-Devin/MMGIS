/**
 * SwathCatalogueAPI — backend plugin lifecycle.
 *
 ...
 */
const router = require('./routes/swathCatalogueAPI')
// Uncomment to create/migrate models/swathCatalogueAPI.js's table on startup. A required
// model is a required table, so a plugin that stores nothing should delete it.
// const { up } = require('./models/swathCatalogueAPI')
...
let setup = {
    ...
    onceInit: (s) => {
        s.app.use(
            ...
            s.ROOT_PATH + '/api/swathCatalogueAPI',
    ...
    // envs: [{ name: 'SWATH_CATALOGUE_API_HOST', required: true }],
}
