/**
 * FovFootprint attachment — unit tests.
 *
...
import FovFootprint from '../fovFootprint.js'
import {
    manifestOf,
    unresolvedModules,
...
test('plugin.json declares a valid layerattachment contract @unit', () => {
    ...
    expect(manifest.attachmentId).toBe('fov_footprint')
    // Settings live on the host, so the form must write where the manifest says
    // this attachment is configured.
    expect(manifest.configPath).toBe('variables.layerAttachments.fovFootprint')
...
test('the declared module resolves and exports make @unit', () => {
    ...
    expect(typeof FovFootprint.make).toBe('function')
})
