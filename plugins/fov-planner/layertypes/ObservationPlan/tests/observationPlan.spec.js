/**
 * ObservationPlan layer type — unit tests.
 *
...
import ObservationPlan from '../observationPlan.js'
...
test('plugin.json declares a valid extending layertype contract @unit', () => {
    ...
    expect(manifest.typeId).toBe('observationplan')
...
test('the module declares surfaces, not renderer operations @unit', () => {
    ...
    for (const key of Object.keys(ObservationPlan)) expect(SURFACES).toContain(key)
})
...
test('fetch returns the features the service gave it @unit', async () => {
    ...
    try {
        const result = await ObservationPlan.source.fetch(
