 ...
 *
 * These import `logic.js`, not `HazardReport.js`: as soon as the handler imports a
...
test('settings are defaulted where they are read @unit', () => {
    const feature = { properties: { name: 'Crater', severity: 'caution', id: 7 } }
    expect(decide(feature, null)).toEqual({
        label: 'Crater',
        severity: 'caution',
        level: 1,
    })
    expect(decide(feature, { labelProp: 'id', severityProp: 'nope' })).toEqual({
        label: '7',
        severity: 'nominal',
        level: 0,
    })
